...
import contactFriendship from "./lib/pri/contact-friendship.ts";
import contactInvite from "./lib/pri/contact-invite.ts";
...
import identityKey from "./lib/pri/identity-key.ts";
...
import phone from "./lib/pri/phone.ts";
...
async function route(
  ...
  if (path === `${PRE}/hello`) {
  ...
  } else if (path.match(`^${PRE}/contact/friendship/`)) {
    return await contactFriendship(req, path, identityId);
  } else if (path.match(`^${PRE}/contact/invite/`)) {
    return await contactInvite(req, path, identityId);
  ...
  } else if (path.match(`^${PRE}/identity/key/`)) {
    return await identityKey(req, path, identityId);
  } else if (path.match(`^${PRE}/identity/`)) {
    return await identity(req, path, identityId);
    ...
    return await phone(req, path, identityId);
